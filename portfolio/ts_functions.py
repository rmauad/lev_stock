...
#             portfolio_results[q][f'model_{i+1}'].bse['const'],
#             portfolio_results[q][f'model_{i+1}'].conf_int().loc['const']
#         ) for q in quantiles]
...
# def format_coefficient(coef, tval, pval, se, conf_int):
#     stars = ''
#     if pval < 0.01:
#         stars = '^{***}'
#     elif pval < 0.05:
...
#     elif pval < 0.1:
#         stars = '^{*}'
#     return f"${coef:.3f}${stars} \\\\ ({se:.3f}) \\\\ [{conf_int[0]:.3f}, {conf_int[1]:.3f}]"
...
def create_latex_table(portfolio_results, portfolio_name, filename):
    ...
    for i, model in enumerate(models):
        model_results = [format_coefficient(
            portfolio_results[q][f'model_{i+1}'].params['const'],
            portfolio_results[q][f'model_{i+1}'].tvalues['const'],
            portfolio_results[q][f'model_{i+1}'].bse['const'],  # Added standard error
            portfolio_results[q][f'model_{i+1}'].conf_int().loc['const'].tolist(),  # Added confidence interval
            portfolio_results[q][f'model_{i+1}'].pvalues['const']
        ) for q in quantiles]
